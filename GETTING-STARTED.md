...
4. 在「局部重绘」框写要改成什么，点 **局部重绘**。只有框内会被重画。
...
## 5.（可选）在 Codex 里用 AI 对话驱动

把 Easel 装进 Codex（personal marketplace 加一条指向 `./plugins/easel`），然后 `codex plugin add easel@personal`，开新对话，就能让 Codex 用工具直接在画布上生成/编辑。详见 [README.md](README.md)。
...
- **外部图「局部重绘」偶发失败**：接口要求宽高是 16 的倍数；Easel 会自动把外部图对齐到 16 的倍数再处理，仍失败的话多半是图太小或太大。
