...
async function saveEaselAsset(pageId, buffer, ext = '.png') {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/generate', async (req, res) => {
        ...
        try {
          ...
          const apiKey = resolveImageApiKey(body)
          if (!apiKey) {
            sendJson(res, 400, {
              error: 'No image API key. Set EASEL_IMAGE_API_KEY (or COWART_IMAGE_API_KEY / OPENAI_API_KEY), then restart.'
            })
            return
          }
          const pageId = nonEmptyString(body.pageId) || 'page:page'
          const sizeObj =
      ...
      server.middlewares.use('/api/edit', async (req, res) => {
        ...
        try {
          ...
          if (nonEmptyString(body.sourceSrc)) {
            const filePath = localAssetFilePathFromUrl(body.sourceSrc)
            if (!filePath) {
              sendJson(res, 400, { error: 'Invalid sourceSrc' })
              return
            }
            sourceBuffer = await readFile(filePath)
            mimeType = mimeTypes.get(extname(filePath).toLowerCase()) ?? 'image/png'
          } else if (nonEmptyString(body.sourceDataUrl)) {
            const parsed = parseDataUrl(body.sourceDataUrl)
            if (!parsed) {
              sendJson(res, 400, { error: 'Invalid sourceDataUrl' })
              return
            }
